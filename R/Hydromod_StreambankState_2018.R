...
#    Streambed State = B (intermediate):             Vertical Rating needs Armoring Potential, Grade Control, D50, Slope, Area, Precipitation (d50 is needed).
#
#
...
#        a. Armoring (assessed in field).  Field crews determine if bank is fully armored or not.  Fully Armored sites don't need the other assessments.
...
#
#   Lateral susceptibility determination for rating #2 from flow chart had to be changed from using bank angle to bank height (8May2019)
#   The risk threshold is determined for the bank angle, and the bank height is compared to thie risk threshold to see if the threshold is exceeded.
#   The code had to be revised to use bank height for the comparison.  Here are excerpts from the corrected script:
#   if (StationCodeCurrent3$BankHeightL1 <= StationCodeCurrent3$Mass.RiskThresh.L1)
#   if (StationCodeCurrent3$BankHeightL1 > StationCodeCurrent3$Mass.RiskThresh.L1)
#
#
#
#
# NOTES
# Particle size category      Median(mm)     Range(mm), p8 of Hydromod field manual
# Hard Pan      (HP)             5660
# Large boulder (XP)             2500
# Small boulder (SB)              625         >256
# Cobble        (CB)              157         >64 - 256
# Gravel coarse (GC)               40         >2 - 64
# Gravel fine   (GF)                9
# Sand          (SA)                1.03      >0.0625 - 2
# Fines         (FN)                0.03      silt >0.004 - 0.0625, clay <=0.004
#
#########################################
...
#AllHydromod     <-read.csv("Z:/SMCStreamMonitoringProgram/Data/2018_Data/Hydromod/Weston/COSD and RCFCD_Weston_2018.csv" , stringsAsFactors = F)


getSusceptibility <- function(AllHydromod) {
...
AllHydromod$StreambedState    <- ifelse(AllHydromod$StreambedState    == " A", "A", AllHydromod$StreambedState)
AllHydromod$StreambedState    <- ifelse(AllHydromod$StreambedState    == " B", "B", AllHydromod$StreambedState)
AllHydromod$StreambedState    <- ifelse(AllHydromod$StreambedState    == " C", "C", AllHydromod$StreambedState)
AllHydromod$ArmoringPotential <- ifelse(AllHydromod$ArmoringPotential == " A", "A", AllHydromod$ArmoringPotential)
AllHydromod$ArmoringPotential <- ifelse(AllHydromod$ArmoringPotential == " B", "B", AllHydromod$ArmoringPotential)
AllHydromod$ArmoringPotential <- ifelse(AllHydromod$ArmoringPotential == " C", "C", AllHydromod$ArmoringPotential)
AllHydromod$GradeControl      <- ifelse(AllHydromod$GradeControl      == " A", "A", AllHydromod$GradeControl)
AllHydromod$GradeControl      <- ifelse(AllHydromod$GradeControl      == " B", "B", AllHydromod$GradeControl)
AllHydromod$GradeControl      <- ifelse(AllHydromod$GradeControl      == " C", "C", AllHydromod$GradeControl)
    ...
    StationCodeCurrent$VertSuscept <- "LOW"
        ...
        if (StationCodeCurrent$StreambedState == "B" | StationCodeCurrent$StreambedState == "" | is.na(StationCodeCurrent$StreambedState)) {
          if (StationCodeCurrent$ArmoringPotential == "" | StationCodeCurrent$GradeControl == "" | StationCodeCurrent$D50 == "" | StationCodeCurrent$INDEX == "" |
            ...
            #  TROUBLE: Blank D50s are replaced with -88 in code above.  This leads to Screening Index Score (SIS) = 6 in the code below
            #     An SIS = 6 is supposed to be used for boulder/hardpan or intermediate D50s.  How was it determined to automatically give blanks a SIS=6?
            ...
            #
            StationCodeCurrent$SIS <- ifelse((StationCodeCurrent$D50 > 256 | StationCodeCurrent$D50 < 0), 6,      #Boulder/hardpan or indeterminate d50 (Screening Index Score = "B")
    ...
    StationCodeCurrent3$LatRating.L1 <- "Unk"
      ...
      if(StationCodeCurrent3$LateralSusceptibilityL == 2){                                            #Laterally adjustable, but not occurring & Moderately or well-consolidated
        if (StationCodeCurrent3$VertRating == "" | is.na(StationCodeCurrent3$VertRating) | StationCodeCurrent3$VWI == "" | is.na(StationCodeCurrent3$VWI) |
          ...
          if (StationCodeCurrent3$BankHeightL1 <= StationCodeCurrent3$Mass.RiskThresh.L1){                       #  <=10% risk of mass wasting
  ...
  if (StationCodeCurrent3$LateralSusceptibilityL == "" | is.na(StationCodeCurrent3$LateralSusceptibilityL)){ #No lateral susceptibility number written in field
    ...
    if(StationCodeCurrent3$LateralSusceptibilityL == 2){                                            #Laterally adjustable, but not occurring & Moderately or well-consolidated
      if (StationCodeCurrent3$VertRating == "" | is.na(StationCodeCurrent3$VertRating) | StationCodeCurrent3$VWI == "" | is.na(StationCodeCurrent3$VWI) |
        ...
        if (StationCodeCurrent3$BankHeightL2 <= StationCodeCurrent3$Mass.RiskThresh.L2){                       #  <=10% risk of mass wasting
          if (StationCodeCurrent3$VertRating <= 7){                                                   #      Vertical rating <high
  ...
  if (StationCodeCurrent3$LateralSusceptibilityL == "" | is.na(StationCodeCurrent3$LateralSusceptibilityL)){ #No lateral susceptibility number written in field
    ...
    if(StationCodeCurrent3$LateralSusceptibilityL == 2){                                            #Laterally adjustable, but not occurring & Moderately or well-consolidated
      if (StationCodeCurrent3$VertRating == "" | is.na(StationCodeCurrent3$VertRating) | StationCodeCurrent3$VWI == "" | is.na(StationCodeCurrent3$VWI) |
        ...
        if (StationCodeCurrent3$BankHeightL3 <= StationCodeCurrent3$Mass.RiskThresh.L3){                       #  <=10% risk of mass wasting
          if (StationCodeCurrent3$VertRating <= 7){                                                   #      Vertical rating <high
  ...
  if (StationCodeCurrent3$LateralSusceptibilityR == "" | is.na(StationCodeCurrent3$LateralSusceptibilityR)){ #No lateral susceptibility number written in field
    ...
    if(StationCodeCurrent3$LateralSusceptibilityR == 2){                                            #Laterally adjustable, but not occurring & Moderately or well-consolidated
      if (StationCodeCurrent3$VertRating == "" | is.na(StationCodeCurrent3$VertRating) | StationCodeCurrent3$VWI == "" | is.na(StationCodeCurrent3$VWI) |
        ...
        if (StationCodeCurrent3$BankHeightR1 <= StationCodeCurrent3$Mass.RiskThresh.R1){                       #  <=10% risk of mass wasting
          if (StationCodeCurrent3$VertRating <= 7){                                                   #      Vertical rating <high
  ...
  if (StationCodeCurrent3$LateralSusceptibilityR == "" | is.na(StationCodeCurrent3$LateralSusceptibilityR)){ #No lateral susceptibility number written in field
    ...
    if(StationCodeCurrent3$LateralSusceptibilityR == 2){                                            #Laterally adjustable, but not occurring & Moderately or well-consolidated
      if (StationCodeCurrent3$VertRating == "" | is.na(StationCodeCurrent3$VertRating) | StationCodeCurrent3$VWI == "" | is.na(StationCodeCurrent3$VWI) |
        ...
        if (StationCodeCurrent3$BankHeightR2 <= StationCodeCurrent3$Mass.RiskThresh.R2){                       #  <=10% risk of mass wasting
          if (StationCodeCurrent3$VertRating <= 7){                                                   #      Vertical rating <high
    ...
    if (StationCodeCurrent3$LateralSusceptibilityR == "" | is.na(StationCodeCurrent3$LateralSusceptibilityR)){ #No lateral susceptibility number written in field
      ...
      if(StationCodeCurrent3$LateralSusceptibilityR == 2){                                            #Laterally adjustable, but not occurring & Moderately or well-consolidated
        if (StationCodeCurrent3$VertRating == "" | is.na(StationCodeCurrent3$VertRating) | StationCodeCurrent3$VWI == "" | is.na(StationCodeCurrent3$VWI) |
          ...
          if (StationCodeCurrent3$BankHeightR3 <= StationCodeCurrent3$Mass.RiskThresh.R3){                       #  <=10% risk of mass wasting
            if (StationCodeCurrent3$VertRating <= 7){                                                   #      Vertical rating <high
...
#write.csv(StationCodeCurrent4, file="Z:/SMCStreamMonitoringProgram/Data/2018_Data/Hydromod/Weston/COSD and RCFCD_Weston_2018_Vertical and Lateral Susceptibility.csv")
  return(StationCodeCurrent4)
